...
//  barnyard.push_back(al);
  ...
  for (int i=0; i<barnyard.size(); ++i) {
    ...
    //Dynamic cast is a function that takes a type and tries to convert it to the proper type, returns 0 or nullptr if unsuccessful
...
//We could make another class called Acrocat that inherits from Cat, but it doesnt falll and break their legs
// The dynamic cast will stil succedd on that subclass. It wont catch the subclass
//Polymorphism is still the most relaiable and least error prone eay to do this 