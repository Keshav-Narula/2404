...
bool List::get(string name, Student** stu){
    for (int i = 0; i < size; ++i){
        if (name == elements[i]->getName()){
            *stu = elements[i]; //Derefence the double pointer
            return true;
        }
    }
    return false;
...
//Now that you have a collection class you will not delete the class, just remove from this specific list, Who ever owns the list decides if they want that studnet deleted or not
//Dont delete students since they could be shared between different lists
...
//Passed by reference, looped thorugh the list and 
