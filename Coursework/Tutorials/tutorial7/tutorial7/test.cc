...
int main() {
    ...
    switch (choice) {
        case 0:
            return franchiseIdTest();
        case 1:
            return driverIdTest();
        case 2:
            return pickUpOrderTest();
    }
...
int pickUpOrderTest() {
    ...
    cout << endl
         << "Adding 3 orders to franchise..." << endl;
    f->addOrder(new Order("alice", 2, Location()));
    f->addOrder(new Order("bucky", 3, Location()));
    f->addOrder(new Order("charlie", 1, Location()));

    if (f->getNumOrders() != 3) {
        cout << "Franchise does not have 3 orders, has " << f->getNumOrders() << ", test failed!" << endl;
        return 1;
    }

    cout << endl
         << "Driver picking up 2 orders..." << endl;

    d->addOrder(f->getNextOrder());
    d->addOrder(f->getNextOrder());

    if (d->getNumOrders() != 2) {
        cout << "Driver does not have 2 orders, has " << d->getNumOrders() << " instead, test failed!" << endl;
        return 1;
    }

    if (f->getNumOrders() != 1) {
        cout << "Franchise does not have 1 order, has " << f->getNumOrders() << ", test failed!" << endl;
        return 1;
    }
    ...
    cout << endl
         << "Delivering 2 orders..." << endl;
    delete d->getNextOrder();
    delete d->getNextOrder();

    if (d->getNumOrders() != 0) {
        cout << "Driver does not have 0 orders, has " << d->getNumOrders() << " instead, test failed!" << endl;
        return 1;
    }

    cout << endl
         << "Driver picking up order" << endl;

    d->addOrder(f->getNextOrder());

    if (d->getNumOrders() != 1) {
        cout << "Driver does not have 1 order, has " << d->getNumOrders() << " instead, test failed!" << endl;
        return 1;
    }

    if (f->getNumOrders() != 0) {
        cout << "Franchise does not have 0 orders, has " << f->getNumOrders() << ", test failed!" << endl;
        return 1;
    }
